...
(function () {
  ...
  const ROLE_META = {
    USER: { label: 'Citizen', icon: '👤', path: 'citizen-dashboard.html', color: '#16a34a' },
  ...
  const Auth = {
    ...
    require(role) {
      const s = this.session();
      if (!s) { nav('citizen-login.html'); return null; }
      if (role && s.role !== role) {
        const target = ROLE_META[s.role] && ROLE_META[s.role].path;
        nav(target || 'citizen-login.html');
        return null;
      }
      return s;
    },

    switchRole() { localStorage.removeItem(SESSION_KEY); nav('citizen-login.html'); },
  };
