...
(function () {
  ...
  panels.forEach((panel) => {
    ...
    function validateConfirm() {
      ...
      if (confirmErr) confirmErr.hidden = !(mode === 'register' && confirmInput.value && !ok);
    }
    if (confirmInput) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (mode === 'register') {
        if (!phone || !password) { setErr(t('auth.errFillPhone')); return; }
      } else if (!email || !password) {
        setErr(t('auth.errFill'));
        return;
      }
      if (mode === 'register') {
        if (password.length < 6) { setErr(t('auth.errPassLen')); return; }
        if (!confirmInput || !confirmInput.value) {
          setErr(t('auth.errConfirmFill'));
